...
grafico.analiseEstatistica_Resumo = function(tabela) {
  ...
  sub = sprintf("h2: %s, Produtividade media: %s", round(correlation[[1]],2) , round(model.mean,2))
  
  ggplot(Y1, aes(x=fct_reorder(gid,mean), y=mean, label=gid)) + 
    geom_bar(stat='identity', aes(fill=type), width=.5) +
    scale_fill_manual(name="Produtividade", 
...
grafico.analiseEstatistica_Unitario = function(tabela, site = "") {
  ...
  ggplot(solo, aes(x=fct_reorder(gid_plot,mean), y=mean, label=gid_plot)) + 
    geom_bar(stat='identity', aes(fill=type), width=.5) +
    scale_fill_manual(name="Produtividade", 
                      labels = c("Acima media geral", "Abaixo media geral"), 
                      values = c("above" = "#7cb342", "below" = "#e53935")) + 
...
grafico.GraficoLinhas = function(dados){
  ...
  #Gerando grafico 
  p = ggplot(data = NULL) + 
    geom_line(data = dados,aes(x = site, y = mean,group = gid, colour = gid)) +
    geom_point(data = dados_mean,aes(x = site,y = Mean),shape = 17,size = 3) +
